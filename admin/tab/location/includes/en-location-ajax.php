...
use EnUvs\EnUvsDropshipTemplate;
use EnUvs\EnUvsWarehouse;
use EnUvs\EnUvsWarehouseTemplate;

if (!class_exists('EnUvsLocationAjax')) {

    class EnUvsLocationAjax {

        public function __construct() {
            add_action('wp_ajax_nopriv_en_uvs_location_save_form_data', [$this, 'en_uvs_location_save_form_data']);
            add_action('wp_ajax_en_uvs_location_save_form_data', [$this, 'en_uvs_location_save_form_data']);

            add_action('wp_ajax_nopriv_en_uvs_get_location', [$this, 'en_uvs_get_location']);
            add_action('wp_ajax_en_uvs_get_location', [$this, 'en_uvs_get_location']);

            add_action('wp_ajax_nopriv_en_uvs_location_delete_row', [$this, 'en_uvs_location_delete_row']);
            add_action('wp_ajax_en_uvs_location_delete_row', [$this, 'en_uvs_location_delete_row']);
        }

        /**
         * Get response from api when we sent zip code
         */
        public function en_uvs_get_location() {
            if (isset($_POST['en_uvs_location_zip']) && strlen($_POST['en_uvs_location_zip']) > 4) {
                $get_address = EnUvsDistance::get_address(sanitize_text_field($_POST['en_uvs_location_zip']), 'address');
            ...
            echo wp_json_encode(['message' => EN_UVS_709]);
            exit;
        }
         ...
         */
        public function en_uvs_location_delete_row() {
            global $wpdb;
            ...
            if ($location === 'warehouse') {
                $en_location_html = EnUvsWarehouseTemplate::en_load();
                $en_target_location = '.en_location_warehouse_main_div';
            } else if ($location === 'dropship') {
                $en_location_html = EnUvsDropshipTemplate::en_load();
         ...
         */
        public function en_uvs_location_save_form_data() {
            global $wpdb;
            $post_data = [];
            ...
            if (isset($post_data['id'], $post_data['location'])) {
                ...
                if ($location === 'warehouse') {
                    ...
                    $en_location_template_obj = new EnUvsWarehouseTemplate();
                    $en_target_location = '.en_location_warehouse_main_div';
                    $validate = ['zip', 'city', 'state', 'country', 'location'];
                } else {
                    $location_step = 'Dropship';
                    $en_location_template_obj = new EnUvsDropshipTemplate();
                    $en_target_location = '.en_location_dropship_main_div';
                ...
                $en_location_data = EnUvsWarehouse::get_data($en_intersected_data);
