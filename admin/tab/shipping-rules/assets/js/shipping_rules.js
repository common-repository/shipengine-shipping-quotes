jQuery(document).ready(function () {
	const form = jQuery('#add_shipping_rule_uvs');
	if (form) {
		form.on('submit', saveShippingRuleUvs);
	}
	...
	jQuery('#en_sr_add_shipping_rule_btn').on('click', function () {
		...
		togglePopupVisibilityUvs(true);
	});
	jQuery('.en_close_sr_popup').on('click', function () {
		togglePopupVisibilityUvs();
	});
	...
	jQuery(
		...
		if (this.checked) {
			if (id === 'filter_by_weight') removeDataAttributeUvs('en_sr_weight_from');
			else if (id === 'en_sr_filter_price') removeDataAttributeUvs('en_sr_price_from');
			else if (id === 'filter_by_quantity') removeDataAttributeUvs('en_sr_quantity_from');
			else if (id === 'filter_by_product_tag')
				removeDataAttributeUvs('en_sr_product_tags_list');
		} else {
			if (id === 'filter_by_weight') setDataAttributeUvs('en_sr_weight_from');
			else if (id === 'en_sr_filter_price') setDataAttributeUvs('en_sr_price_from');
			else if (id === 'filter_by_quantity') setDataAttributeUvs('en_sr_quantity_from');
			else if (id === 'filter_by_product_tag')
				setDataAttributeUvs('en_sr_product_tags_list');
		}
	});

	jQuery(document).on('click', '.uvs_sr_edit_link', function () {
		const id = jQuery(this).attr('data-id');
		editShippingRuleUvs(id, this);
	});
	jQuery(document).on('click', '.uvs_sr_delete_link', function () {
		const id = jQuery(this).attr('data-id');
		deleteShippingRuleUvs(id, this);
	});
	jQuery(document).on('click', '.uvs_sr_status_link', function (e) {
		updateShippingRuleStatusUvs(this);
	});

	function setDataAttributeUvs(...attr_ids) {
		for (let id of attr_ids) {
			jQuery('#' + id).attr('data-optional', '1');
		}
	}

	function removeDataAttributeUvs(...attr_ids) {
		for (let id of attr_ids) {
			jQuery('#' + id).removeAttr('data-optional');
		}
	}

	function enSRValidateInputUvs(form_id = '') {
		let is_valid = true;
		let input = (response = errorText = optional = errorElement = to = '');

		jQuery(form_id + " input[type='text']").each(function () {
			input = jQuery(this).val();
			response = validateStringUvs(input);
	...
	function validateStringUvs(string) {
		return string == '' ? 'empty' : true;
	}

	function saveShippingRuleUvs(e) {
		e.preventDefault();

		const is_valid = enSRValidateInputUvs('#add_shipping_rule_uvs');
		if (!is_valid) {
		...
		const postData = {
			action: 'en_uvs_save_shipping_rule',
		...
		jQuery.ajax({
			...
			success: function (data) {
				...
				const rowMarkup = getRowMarkupUvs({ ...data, ...postData });
        jQuery('.uvs_empty_sr_row').hide();

				if (data.insert_qry == 1) {
					const newRowMarkup = `<tr id='uvs_sr_row_${data.id}' class="uvs_sr_row">${rowMarkup}</tr>`;
					jQuery('#en_uvs_shipping_rules_list tbody').append(newRowMarkup);
					togglePopupVisibilityUvs();
					jQuery('.eniture_uvs_sr_created').show('slow').delay(3000).hide('slow');
				} else if (data.update_qry == 1) {
					jQuery(`#uvs_sr_row_${data.id}`).html(rowMarkup);
					togglePopupVisibilityUvs();
					jQuery('.eniture_uvs_sr_updated').show('slow').delay(3000).hide('slow');
				} else if (data.error) {
	...
	function editShippingRuleUvs(rule_id, elem) {
		form[0]?.reset();
		jQuery('.en_sr_err').html('');
		setDataAttributeUvs(
			'en_sr_weight_from',
		...
		const postForm = {
			action: 'en_uvs_edit_shipping_rule',
			edit_id: rule_id,
		};
		...
		jQuery.ajax({
			...
			success: function (data) {
				...
				if (data) {
					...
					settings?.filter_by_weight === 'true' &&
						removeDataAttributeUvs('en_sr_weight_from');
					...
					settings?.filter_by_price === 'true' &&
						removeDataAttributeUvs('en_sr_price_from');
					...
					settings?.filter_by_quantity === 'true' &&
						removeDataAttributeUvs('en_sr_quantity_from');
	...
	function deleteShippingRuleUvs(id, elem) {
		const postForm = {
			action: 'en_uvs_delete_shipping_rule',
			delete_id: id,
		};
		...
		jQuery.ajax({
			...
			success: function (data) {
				jQuery('#uvs_sr_row_' + id).remove();
				jQuery('.eniture_uvs_sr_deleted').show('slow').delay(3000).hide('slow');
        if (jQuery('.uvs_sr_row')?.length) return false;

         if (jQuery('.uvs_empty_sr_row')?.length) {
          jQuery('.uvs_empty_sr_row').show();
        } else {
          const newRowMarkup = `<tr class="new_warehouse_add uvs_empty_sr_row" data-id=0><td class="" colspan="5" style="text-align: center;">No data found!</td></tr>`;
          jQuery('#en_uvs_shipping_rules_list tbody').append(newRowMarkup);
        }
			},
			error: function (error) {},
	...
	function updateShippingRuleStatusUvs(elem) {
		const rule_id = jQuery(elem).attr('data-id');
		const is_active = jQuery(elem).attr('data-status');

		// Submit the form to save settings
		const postData = {
			action: 'en_uvs_update_shipping_rule_status',
			rule_id,
	...
	function togglePopupVisibilityUvs(show = false) {
