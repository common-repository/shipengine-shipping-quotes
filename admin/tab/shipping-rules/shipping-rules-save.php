...
namespace EnUvsShippingRulesAjaxReq;

if (!class_exists("EnUvsShippingRulesAjaxReq")) {

    class EnUvsShippingRulesAjaxReq
    {
        /**
        ...
        {
            add_action('wp_ajax_nopriv_en_uvs_save_shipping_rule', array($this, 'save_shipping_rule_uvs_ajax'));
            add_action('wp_ajax_en_uvs_save_shipping_rule', array($this, 'save_shipping_rule_uvs_ajax'));

            add_action('wp_ajax_nopriv_en_uvs_edit_shipping_rule', array($this, 'edit_shipping_rule_uvs_ajax'));
            add_action('wp_ajax_en_uvs_edit_shipping_rule', array($this, 'edit_shipping_rule_uvs_ajax'));

            add_action('wp_ajax_nopriv_en_uvs_delete_shipping_rule', array($this, 'delete_shipping_rule_uvs_ajax'));
            add_action('wp_ajax_en_uvs_delete_shipping_rule', array($this, 'delete_shipping_rule_uvs_ajax'));

            add_action('wp_ajax_nopriv_en_uvs_update_shipping_rule_status', array($this, 'update_shipping_rule_status_uvs_ajax'));
            add_action('wp_ajax_en_uvs_update_shipping_rule_status', array($this, 'update_shipping_rule_status_uvs_ajax'));
        }
         ...
         * @global $wpdb
         */
        function save_shipping_rule_uvs_ajax()
        {
            global $wpdb;
            ...
            $qry = "SELECT * FROM " . $wpdb->prefix . "eniture_uvs_shipping_rules WHERE name = '" . $data['name'] . "'"; 
            ...
            if (!empty($get_shipping_rule_id)) {
                ...
                $update_qry = $wpdb->update(
                    $wpdb->prefix . 'eniture_uvs_shipping_rules', $data, array('id' => $get_shipping_rule_id)
                );
                ...
                if (!empty($get_shipping_rule)) {
                    ...
                    $data['settings'] = json_encode($data['settings']);
                    $insert_qry = $wpdb->insert($wpdb->prefix . 'eniture_uvs_shipping_rules', $data);
         ...
         * @global $wpdb
         */
        function edit_shipping_rule_uvs_ajax()
        {
            global $wpdb;
            $get_shipping_rule_id = (isset($_POST['edit_id']) && intval($_POST['edit_id'])) ? $_POST['edit_id'] : "";
            $shipping_rules_list = $wpdb->get_results(
                "SELECT * FROM " . $wpdb->prefix . "eniture_uvs_shipping_rules WHERE id=$get_shipping_rule_id"
            );
         ...
         * @global $wpdb
         */
        function delete_shipping_rule_uvs_ajax()
        {
            global $wpdb;
            $get_shipping_rule_id = (isset($_POST['delete_id']) && intval($_POST['delete_id'])) ? $_POST['delete_id'] : "";
            $qry = $wpdb->delete($wpdb->prefix . 'eniture_uvs_shipping_rules', array('id' => $get_shipping_rule_id));
         ...
         * @global $wpdb
         */
        function update_shipping_rule_status_uvs_ajax()
        {
            global $wpdb;
