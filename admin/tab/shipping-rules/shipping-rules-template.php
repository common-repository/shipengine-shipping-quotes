...
if (!function_exists('shipping_rules_template')) {
    function shipping_rules_template($action = false)
    {
      ...
      $shipping_rules_list = $wpdb->get_results(
          "SELECT * FROM " . $wpdb->prefix . "eniture_uvs_shipping_rules"
      );

      ?>
        <div>
          <table class="en_location_table" id="en_uvs_shipping_rules_list">
            <!-- Table Headings -->
            <thead>
                  <tr>
                      <th>Rule Name</th>
              ...
              <?php
                if (count($shipping_rules_list) > 0) {
                  ...
                  foreach ($shipping_rules_list as $rule) {
                    ...
                    ?>
                      <tr id="uvs_sr_row_<?php echo (isset($rule->id)) ? esc_attr($rule->id) : ''; ?>" class="uvs_sr_row">
                        <td><?php echo $rule->name; ?></td>
                        <td><?php echo $rule->type; ?></td>
                        <td><?php echo isset($rule->settings['filter_name']) ? esc_html($rule->settings['filter_name']) : ''; ?></td>
                        <td>
                          <a href="#" class="uvs_sr_status_link" data-id="<?php echo (isset($rule->id)) ? esc_attr($rule->id) : ''; ?>" data-status="<?php echo $rule->is_active; ?>"><?php echo $rule->is_active ? 'Yes' : 'No'; ?></a>
                        </td>
                        <td>
                          <!-- Edit rule link -->
                          <a href="#" class="uvs_sr_edit_link" data-id="<?php echo (isset($rule->id)) ? esc_attr($rule->id) : ''; ?>">
                            <img src="<?php echo plugins_url(); ?>/small-package-quotes-ups-edition/warehouse-dropship/wild/assets/images/edit.png" title="Edit">
                          </a>
                          <!-- Delete rule link -->
                          <a href="#" class="uvs_sr_delete_link" data-id="<?php echo (isset($rule->id)) ? esc_attr($rule->id) : ''; ?>">
                  ...
                  ?>
                    <tr class="new_warehouse_add uvs_empty_sr_row" data-id=0>
...
<div class="en_uvs_shipping_rules_setting_section">
  <br />
  <!-- Add rule button -->
  ...
  <div class="en_popup_sr_overlay">
      <div class="en_popup_sr_form">
        ...
        <div class="en_popup_sr_content">
          ...
          <form id="add_shipping_rule_uvs" role="form">
              ...
              <!-- Form submit button -->
              <div class="eniture_uvs_save_sr_form">
