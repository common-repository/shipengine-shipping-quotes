...
namespace EnUvsOrderExport;

/**
 * Get and save settings.
 * Class EnUvsOrderExport
 * @package EnUvsOrderExport
 */
if (!class_exists('EnUvsOrderExport')) {
    ...
    class EnUvsOrderExport
    {
        ...
        public function en_orders_sending($orders, $en_orders_ids)
        {
            $post_data = [
                ...
                'licenseKey' => get_option('uvs_small_licence_key'),
                'currencyUnit' => get_option('woocommerce_currency'),
                'platform' => 'wordpress',
                'orders' => $orders,
            ];

            $response = \EnUvs\EnUvsCurl::en_uvs_sent_http_request(EN_UVS_ORDER_EXPORT_HITTING_URL, $post_data, 'POST', 'Export Order');
