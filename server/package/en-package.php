...
use EnUvs\EnUvsDistance;
use EnUvs\EnUvsProductDetail;
use EnUvs\EnUvsQuoteSettings;
use EnUvs\EnUvsReceiverAddress;
use EnUvs\EnUvsWarehouse;

/**
 * Get items detail from added product in cart|checkout page.
 * Class EnUvsPackage
 * @package EnUvsPackage
 */
if (!class_exists('EnUvsPackage')) {
    ...
    class EnUvsPackage {
        ...
        static public function en_package_converter($package) {
            $product_detail_obj = new EnUvsProductDetail();
            ...
            self::$receiver_address = EnUvsReceiverAddress::get_address();
            $insurance_plan_status = apply_filters("uvs_plans_suscription_and_features", 'insurance');
            $transit_days_status = apply_filters("uvs_plans_suscription_and_features", 'transit_days');
            foreach ($package['contents'] as $key => $product) {
                if (isset($product['data'])) {
                    ...
                    $isExemptGndTransit = 0;
		            if(is_bool($transit_days_status) === true && $transit_days_status){
                        $exmpt_gnd_transit_flag = maybe_unserialize(get_post_meta(self::$post_id, '_en_exempt_ground_transit_restriction', true));
                        if(!empty($exmpt_gnd_transit_flag) && $exmpt_gnd_transit_flag == 'yes'){
                            $isExemptGndTransit = 1;
                        }
                    }
                    ...
                    $shipment_type = EN_UVS_DECLARED_FALSE;
                    $origin_zip_code = EN_UVS_DECLARED_ZERO;
                    // Micro Warehouse
                    ...
                    foreach ($en_product_fields as $en_field_key => $en_custom_product) {
                        ...
                        if (is_callable(array(__CLASS__, $en_function_trigger)) && strlen($is_line_item) > EN_UVS_DECLARED_ZERO) {
                            ...
                            $suscription_and_features = apply_filters(
                                    "uvs_plans_suscription_and_features", $is_plans
                            );
                            ...
                            if (!is_array($en_location_value) && strlen($en_location_value) > EN_UVS_DECLARED_ZERO) {
                            ...
                            if (isset($en_location_value['senderZip']) &&
                                ...
                                switch (EN_UVS_DECLARED_TRUE) {
                                    case $shipping_class == 'ltl_freight':
                                        $shipment_type = EN_UVS_DECLARED_TRUE;
                                        self::en_set_ltl_shipment($origin_zip_code);
                                        self::$en_request['LTL_FREIGHT'] = EN_UVS_DECLARED_ONE;
                                        break;
                                    case $shipment_weight > EN_UVS_SHIPMENT_WEIGHT_EXCEEDS_PRICE &&
                                    EN_UVS_SHIPMENT_WEIGHT_EXCEEDS == 'yes':
                                        self::en_set_ltl_shipment($origin_zip_code);
                                        $shipment_type = EN_UVS_DECLARED_TRUE;
                                        $product_weight < EN_UVS_SHIPMENT_WEIGHT_EXCEEDS_PRICE ?
                                                        self::$en_request['shipment_type']['LTL_SMALL'][$origin_zip_code]['SMALL'] = EN_UVS_DECLARED_TRUE : "";
                                        break;
                                    case $product_weight < EN_UVS_SHIPMENT_WEIGHT_EXCEEDS_PRICE:
                                        $shipment_type = EN_UVS_DECLARED_TRUE;
                    ...
                    if($isInsuranceIctive){
                        $origin_address['insuranceEnable'] = '1';
                        $origin_address['prefferedCurrency'] = 'USD';
                    }

                    if($isExemptGndTransit){
                        $origin_address['isExemptGndTransit'] = 1;
                    }
        ...
        static public function en_uvs_reason_quotes_not_returned($reasons) {
            $reasons = !self::$shipment_type ? array_merge($reasons, [EN_UVS_712]) : $reasons;
            return (!self::$origin_zip_code > 0) ? array_merge($reasons, [EN_UVS_713]) : $reasons;
        }
        ...
        static public function en_filter_shipment() {
            ...
            self::$en_request = apply_filters("en_woo_addons_carrier_service_quotes_request", self::$en_request, EN_UVS_SHIPPING_NAME);
            ...
            self::en_uvs_quote_settings_detail();
        ...
        static public function en_set_ltl_shipment($origin_zip_code) {
            self::$en_request['shipment_type']['LTL_FREIGHT'][$origin_zip_code]['LTL'] = self::$en_request['shipment_type']['LTL_SMALL'][$origin_zip_code]['LTL'] = EN_UVS_DECLARED_TRUE;
        }
        ...
        static public function en_set_small_shipment($origin_zip_code) {
            self::$en_request['shipment_type']['LTL_FREIGHT'][$origin_zip_code]['SMALL'] = self::$en_request['shipment_type']['LTL_SMALL'][$origin_zip_code]['SMALL'] = EN_UVS_DECLARED_TRUE;
        }
         ...
         * @return array|false
         */
        static public function en_uvs_sender_location_address($location) {
        ...
        static public function en_is_instore_pickup_enabled($location, $sender_location_address) {
            ...
            $suscription_and_features = apply_filters(
                    "uvs_plans_suscription_and_features", 'instore_pickup_local_delivery'
            );
         ...
         * @return array|false
         */
        static public function en_uvs_receiver_location_address($receiver_address) {
         ...
         * @return array|false
         */
        static public function en_uvs_quote_settings_detail() {
            $delivery_estimate_option = $cutt_off_time = $fulfilment_offset_days = '';
            $active_services = $settings = $shipment_days = [];
            $feature_option = apply_filters("uvs_plans_suscription_and_features", 'cutt_off_time');
            ...
            if(empty($packaging_method)){
                $settings['packagesType'] = 'ship_alone';
            }else if('ship_one_package_70' == $packaging_method){
                $settings['packagesType'] = 'ship_as_one';
                $settings['perPackageWeight'] = '70';
            }elseif('ship_one_package_150' == $packaging_method){
                $settings['packagesType'] = 'ship_as_one';
                $settings['perPackageWeight'] = '150';
            }else{
                $settings['packagesType'] = $packaging_method;
            }
         ...
         */
        static public function en_uvs_get_location($location_id = '') {
            ...
            $en_location = EnUvsWarehouse::get_data($en_where_clause);
            // Micro Warehouse
            if (stripos(implode($all_plugins), 'micro-warehouse-shipping.php')  || is_plugin_active_for_network('micro-warehouse-shipping-for-woocommerce/micro-warehouse-shipping.php')) {
            ...
            if (!empty($en_location) && is_array($en_location)) {
                if (count($en_location) == 1) {
                    return self::en_uvs_sender_location_address(reset($en_location));
                } else {
                    $en_access_level = 'MultiDistance';
                    // receiver address
                    $receiver_address = self::$receiver_address;

                    $receiver_address = self::en_uvs_receiver_location_address($receiver_address);
                    $get_address = json_decode(
                            EnUvsDistance::get_address($en_location, $en_access_level, $receiver_address), true);

                    return (isset($get_address['origin_with_min_dist']) && !empty($get_address['origin_with_min_dist'])) ?
                            self::en_uvs_sender_location_address($get_address['origin_with_min_dist']) : [];
                }
            }
                    ...
                    switch ($enable_dropship) {
                        case 'yes':
                            ...
                            if (stripos(implode($all_plugins), 'micro-warehouse-shipping.php')  || is_plugin_active_for_network('micro-warehouse-shipping-for-woocommerce/micro-warehouse-shipping.php')) {
                                $locations_dropship = maybe_unserialize(get_post_meta(self::$post_id, '_dropship_location', true));
                                if(!empty($locations_dropship)) {
                                    self::$get_minimum_warehouse = self::en_uvs_get_location();
                                    return self::$get_minimum_warehouse;
                                }
                            }
                            return self::en_uvs_get_location(get_post_meta(self::$post_id, self::$product_key_name, true));
                        default:
                            self::$get_minimum_warehouse = self::en_uvs_get_location();
                            return self::$get_minimum_warehouse;
                    }
        ...
        */
        public static function apply_shipping_rules($en_uvs_package)
        {
            if (empty($en_uvs_package)) return false;
    
            global $wpdb;
            $qry = "SELECT * FROM " . $wpdb->prefix . "eniture_uvs_shipping_rules"; 
            ...
            foreach ($rules as $rule) {
                ...
                $is_rule_applied = self::apply_rule($settings, $en_uvs_package);
        ...
        */
        public static function apply_rule($settings, $en_uvs_package)
        {
            $is_rule_applied = false;
    
            $origin_addresses = isset($en_uvs_package['originAddress']) ? array_keys($en_uvs_package['originAddress']) : [];

            if ($settings['apply_to'] == 'cart') {
                $formatted_values = self::get_formatted_values($origin_addresses, $en_uvs_package);
                ...
                foreach ($origin_addresses as $key => $origin_id) {
                    ...
                    $formatted_values = self::get_formatted_values($shipments, $en_uvs_package);
                    $is_rule_applied = self::apply_rule_filters($settings, $formatted_values);
        ...
        */
        public static function get_formatted_values($origin_addresses, $en_uvs_package)
        {
            $formatted_values = ['weight' => 0, 'price' => 0, 'quantity' => 0, 'tags' => []];
    
            foreach ($origin_addresses as $origin) {
                $formatted_values['weight'] += floatval($en_uvs_package['shipment_weight'][$origin]);
                $formatted_values['price'] += floatval($en_uvs_package['product_prices'][$origin]);
                $formatted_values['quantity'] += floatval($en_uvs_package['product_quantities'][$origin]);
                $formatted_values['tags'] = array_merge($formatted_values['tags'], $en_uvs_package['product_tags'][$origin]);
            }
